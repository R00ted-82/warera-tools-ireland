/* ═══════════════════════════════════════════════════════════════════
 *  IRISH FACTORY TAX — TEST PAGE  (standalone #tax-test view, direct-link
 *  only; not linked from any nav during the test phase)
 *
 *  Same live data pipeline as js/irish-tax.js, but clicking a country row
 *  now opens a small menu instead of jumping straight to the worker list:
 *    - View workers        (same drill-down as the live tax page)
 ...
 *  so far this week (days logged, total tax logged, last update).
 *
 ...
 *  CSS classes.
 * ═══════════════════════════════════════════════════════════════════ */
const IrishTaxTestTool = (() => {
  const PAGE_LIMIT     = 100;
  const WAGE_WINDOW_MS = 24 * 3600 * 1000;
  const WAGE_MAX_PAGES = 5;

  const $refresh = document.getElementById('tax2-refresh');
  const $summary = document.getElementById('tax2-summary');
  const $logReport = document.getElementById('tax2-log-report');
  const $table   = document.getElementById('tax2-table');
  const steps    = makeSteps(document.getElementById('tax2-steps'));
  const setStatus = makeStatus(document.getElementById('tax2-status'));

  let loaded = false;
  let nameById = {};   // userId -> username (citizens free; workers resolved)
  let currentLog = null;      // parsed data/tax/current_week.json
  let weekLogs = null;        // [{ weekStart, data }] most-recent-first, lazy-loaded
  ...
  // before it, oldest→newest reversed to newest→oldest. Missing archives
  // (fewer than 5 weeks of history so far) are simply skipped.
